function mlengthSDWBA(mlength_it,file2save,dirname,ActualLength,fileshape,fatness,frequency,theta,stdphase0,freq0,N0,g0,h0,c,noise_realisations)
...
%			waitbar(0,w,sprintf('SDWBA calculations Iteration %d, %d of %d',iter, irealisation, noise_realisations));
            ...
            [BSTS,BSsigma,form_function]=BSTS_SDWBA(frequency,r,a,h,g,phi,[stdphase0 freq0 N0],c,0);

            save(strcat(dirname,'/dataSDWBA/',sprintf('%s%d_%d',file2save,mlength_it,irealisation)),'N0','stdphase0','freq0','BSTS','BSsigma','frequency','phi','form_function','ActualLength','c')
        end
%		close(w);
        ...
        for irealisation=1:noise_realisations
            load(strcat(dirname,'/dataSDWBA/',sprintf('%s%d_%d',file2save,mlength_it,irealisation)))
            BSsigmatot(:,:,irealisation)=BSsigma;
        end
        StandardDeviation=std(BSsigmatot,[],3);
        BSsigma=mean(BSsigmatot,3);
        BSTS=10*log10(BSsigma);
        save(strcat(dirname,'/',sprintf('%s%d',file2save,mlength_it)),'N0','stdphase0','freq0','frequency','BSTS','BSsigma','phi','StandardDeviation','ActualLength','c');
