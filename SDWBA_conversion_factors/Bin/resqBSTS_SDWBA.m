function [TS,sigma,form_function]=BSTS_SDWBA(frequency,r0,a0,h0,g0,phi,initial_parameters,c,wbar)
...
%    waitbar(0,wbar);
%else
%    wbar=waitbar(0,'SDWBA calculations');
%end
...
for ik=1:length(k)
    ...
    if N<=N0
        ...
        delta_x = r0(1,1) ;
        xx = 0:delta_x/(N):delta_x;
        yy = spline(r0(:,1),r0(:,2),xx);
        r = flipud([xx' yy' zeros(length(xx),1)]);
        % r=resample(r0,p,q);
        a=resample(a0,p,q);
    ...
    for i_phi=1:length(phi)
        ...
        %waitbar((i_phi+(ik-1)*length(phi))/length(phi)/length(k),wbar);
    end
end
