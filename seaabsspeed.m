function [Freq,seaAbs, seaC] = seaabsspeed(socib,date_ST, date_ED)

%Get absorption coefficent and sound speed from Slocum glider processed mat
%file created using the SOCIB Matlab code. Put in a start and end date (ie 20180625)and  
...
if nargin == 0
        [FileName,PathName] = uigetfile('*.mat','Pick glider socib mat file to process');
...
socib = fullfile(PathName,FileName); % puts the path and file name together
load(socib, 'data_processed') % load the SOCIB mat file

yrs = datestr(epoch2datenum(data_processed.time),'yyyymmdd') ; %get out the date and make it a number
dte = str2num(yrs); %convert from string to number


mydata = [dte data_processed.temperature data_processed.conductivity data_processed.salinity data_processed.depth];
